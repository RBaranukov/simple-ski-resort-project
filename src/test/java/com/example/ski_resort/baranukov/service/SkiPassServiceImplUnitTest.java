...
public class SkiPassServiceImplUnitTest {
    ...
    public void getSkiPassByIdAndEqualCost() {
        SkiPass skiPass = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(1756.6), null ,null );

        Mockito.when(skiPassRepository.findById(1L)).thenReturn(Optional.of(skiPass));

        SkiPassDTO skiPassDTO = skiPassService.get(1L);
    ...
    public void getAllSkiPassesAndEqualListSize() {
        SkiPass skiPass1 = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(1756.6), null ,null );
        SkiPass skiPass2 = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(0), null ,null );
        List<SkiPass> skiPasses = Stream.of(skiPass1, skiPass2)
                .collect(Collectors.toList());

        Mockito.when(skiPassRepository.findAll()).thenReturn(skiPasses);

        List<SkiPassDTO> skiPassDTOList = skiPassService.getAll();
    ...
    public void saveSkiPassAndEqualCost(){
        SkiPass input = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(1756.6), null ,null );
        SkiPass returned = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(1756.6), null ,null );
    ...
    public void deleteSkiPassTest(){
        ...
        skiPassService.delete(input.getId());
    }
    ...
    public void updateSkiPassTest(){
        ...
        SkiPass returned = new SkiPass(LocalDateTime.now(), BigDecimal.valueOf(0), null ,null );

        Mockito.when(skiPassRepository.findById(1L)).thenReturn(Optional.of(input));
        Mockito.when(skiPassRepository.save(input)).thenReturn(returned);

        SkiPass result = skiPassService.update(input);
